/**
 * ActionScript code for `DefineSprite_421_mc_touko`, frame 3's `DoAction` script.
 ...
 *
 * This is identical to scripts in `DefineSprite_343_mc_kiri/frame_3/DoAction.txt`,
 * `DefineSprite_350_mc_miki/frame_3/DoAction.txt`, and `DefineSprite_369_mc_misato/frame_3/DoAction.txt`.
 *
 * In JavaScript, this translates to a direct assignment between properties
 * of display objects.
 * It assumes `this` refers to the `mc_touko` MovieClip instance, and it has
 * a `charatype` property, and `youko_mc` is a child MovieClip with a `cfftype` property.
 */
function DoAction_DefineSprite_421_mc_touko_frame_3() {
   // Assuming 'this' refers to the mc_touko MovieClip instance.
   // And `youko_mc` is a child MovieClip of 'this' or accessible.
   if (this.youko_mc) {
      this.youko_mc.cfftype = this.charatype; // Assign Touko's charatype to Youko's cfftype.
   }
}
